...
**Reemplazar a un anotado (`replaceRegistration`, CF)**: "no vino Gonza, jugó Nahuel". Botón ⇄ en cada fila de la lista de `MatchDetailPage` (OG/owner/admin del grupo, creador o admin global — mismo permiso que el reenvío del aviso). En una transacción: la inscripción nueva (docId = uid del reemplazo, debe ser miembro del grupo y no estar anotado) **hereda posición, titular/suplente y equipo**; `currentPlayers` no cambia; si el partido ya terminó, las `playerStats` del que no vino **pasan al que jugó** (sin MVP/Muralla) para que pueda recibir votos. La inscripción nueva lleva `replacedRegistrationId`, y así `onRegistrationDeleted` reconoce el reemplazo y **no manda ningún aviso** de baja.

**Registro de bajas (`dropouts`, panel del admin global)**: `onRegistrationDeleted` escribe `kind: 'left'` y `replaceRegistration` escribe `kind: 'replaced'`, con `hoursBeforeMatch` (la anticipación es lo que separa avisar con días de borrarse una hora antes — contarlas igual empuja a no anotarse temprano). Solo gente con cuenta y partidos no terminados. Lo lee **solo** un admin global (`DropoutsReport.vue` en `AdminDashboardPage`, agrupado por jugador: bajas / tarde = titular con <24hs o reemplazado / no vino); nadie escribe desde el cliente. `onMatchDeletedCleanDropouts` borra las filas de un partido borrado, porque `deleteMatch` borra las inscripciones antes que el partido y si no todos quedarían como "se bajaron". **No hay sanción automática**: es información para decidir a mano.
...
**Armado de equipos (`MatchDetailPage`, sección `#equipos`)**: solo con la lista cerrada (`closed`/`full`) y solo OG/owner/admin del grupo o admin global. Dos caminos, que terminan en la MISMA propuesta editable (`teamPreview`) y en el mismo `assignTeams`:
- **"Sugerir equipos"** → `useTeamBalancer.suggestTeams` (fuerza + posición + química). **Los invitados sin cuenta entran al reparto**: antes se los filtraba (`starters.filter(r => r.userId)`) y quedaban fuera de los equipos, justo cuando el que organiza está más apurado. Van con **fuerza neutra** (`playerStrength` ya devuelve 0.5 con `matchesPlayed === 0`) y una **posición sorteada** (`randomGuestPositions()`): sin zona, el balance posicional los repartía a ciegas y podía juntar a los cuatro invitados de un lado. ⚠️ El sorteo se hace **una vez, al armar la lista de jugadores** — nunca dentro de la función de costo, que se evalúa cientos de veces por sugerencia y con una zona cambiante volvería ruido la comparación entre particiones. Cada "Re-sugerir" vuelve a sortear. **El arco queda afuera del sorteo**: ser arquero es un rol declarado, y un arquero imaginario le haría creer al balance que ese lado ya está cubierto. El invitado conserva `userId: null` (el chip no navega a un perfil inexistente y la química de los demás nunca lo encuentra, que es lo correcto: no tiene historial con nadie).
- **"Pegar equipos"** → `src/utils/parseTeamsText.js`, función pura: recibe el texto del chat y los titulares, devuelve a quién le toca cada equipo. Existe porque muchos grupos arman A y B en WhatsApp y recién después abren la app; sin esta puerta había que rearmar la formación chip por chip y lo más probable era que nadie lo hiciera — y entonces el aviso previo (`match_hype`), las estadísticas por equipo y el resultado quedaban con equipos que no fueron los que jugaron. **Lo pegado NO se guarda directo**: cae en `teamPreview`, así un nombre mal escrito en el chat no puede mover a nadie sin que se vea antes. Detalles del parseo: encabezados reconocidos solo si ocupan **todo** el renglón (`Equipo A`/`A:`/`Team 1`/`Suplentes`) —aceptarlos como prefijo partiría un "Equipo A vs Equipo B" por la mitad—; se descartan los renglones del propio "Compartir lista" (emojis de título/sede/hora, el link); **fallback sin encabezados**: dos bloques separados por un renglón en blanco son A y B (con uno o con tres o más no hay forma de adivinar, y la pantalla pide los encabezados). Emparejar nombres va por niveles (exacto → prefijo de palabra → nombre de pila → palabra contenida), nunca por pedazos de palabra (`Ana` no puede caer en `Mariana`), cada inscripto se usa **una sola vez**, y lo que no cierra se dice de frente en el notify: `unmatched` (nombres que no están en la lista), `ambiguous` (dos "Juan" — se toma el primero pero se avisa, porque ahí la app puede estar poniendo al equivocado) y `missing` (anotados que el texto no nombró, que quedan **visibles** en la sección "Sin equipo" del preview: esconderlos haría creer que la formación está completa).

**Reenvío manual del aviso de lista abierta (`resendMatchListNotification`)**: botón "Reenviar aviso" en la **fila de gestión del `DashboardPage`** (junto a Editar / Calcular equipos / Borrar), visible solo mientras el estado **guardado** es `'open'` (`rawStatus` — un partido lleno sigue `open` en Firestore). Antes vivía en `MatchDetailPage` junto a "Compartir lista" y, como se llegaba ahí desde "Calcular equipos", parecía parte del armado de equipos. Existe porque los avisos automáticos (`match_open`, `match_reminder`) salen **una sola vez**; en un grupo donde la lista queda abierta toda la semana ese único push se pierde en el chat y nadie se acuerda de anotarse — esto la "revive" a mano. Manda a los miembros del grupo que todavía no están anotados (mismo patrón `sendFCMToGroupMembers` + `getRegisteredUserIds` que `match_reminder`/`low_signup_alert`).
- **Permiso**: OG u owner/admin del grupo, el creador del partido (aunque no sea OG), o admin global — `assertCanResendMatchListNotification`. A propósito NO reusa `assertCanManageMatchNotifications` (esa solo deja pasar a owner/admin): acá se pidió explícitamente que los OG también puedan tocar el botón.
- **Rate limit "chill", por PARTIDO** (no por usuario, no global): cooldown de `MANUAL_REMINDER_COOLDOWN_MS` (3hs) entre envíos + tope de `MANUAL_REMINDER_MAX_PER_DAY` (2) por día de calendario en huso AR. Se guarda en el propio doc del partido (`manualReminderLastAt/Day/Count`) y se chequea + escribe en **una transacción**, así dos taps casi simultáneos no se cuelan los dos. ⚠️ Esas dos constantes están **duplicadas** en `src/composables/useMatch.js` (`manualReminderStatus`, que deriva el estado del botón de los mismos campos del doc, sin lectura extra) — el límite real lo aplica el backend, el del cliente es solo para no mostrar el botón habilitado cuando ya se sabe que el servidor lo va a rechazar.

**Aviso de "hype" antes del partido (`match_hype`)**: mensaje personalizado a una parte de los titulares un rato antes de jugar. **Espera a que estén los equipos, y esa es toda la idea**: el texto dice "hoy tenés enfrente a Fulano" / "hoy jugás con Fulano", y eso es una afirmación sobre la formación de HOY. Antes se armaba mirando **solo el historial** (`chemistry`/`rivalry`), sin abrir el campo `team` — así que cuando el compañero histórico caía en el equipo contrario, la app mentía. Ahora:
- `assignTeams` (useRegistration) estampa `matches/{id}.teamsAssignedAt` en el **mismo batch** que reparte los `team`, para que no exista el estado "equipos repartidos pero sin sellar". Regla propia (`hasOnly(['teamsAssignedAt','updatedAt'])`, mismo permiso que asignar `team`).
- `runMatchHypeNotify` corre **cada 10 min** y solo avisa cuando pasaron `HYPE_TEAMS_DEBOUNCE_MS` (2 min) desde ese sello, así que el aviso cae **entre 2 y 12 min** después del último retoque. **Cada reasignación lo vuelve a estampar y reinicia la espera**: el debounce no es un margen técnico, es el tiempo que tarda el DT en aceptar la sugerencia y mover dos jugadores a mano. Avisar en el acto mandaría una formación que cambia treinta segundos después.
- **Si los equipos nunca se arman**, a `HYPE_NO_TEAMS_FALLBACK_MS` (1h) del partido sale igual, pero **solo con la racha personal** (`buildStreakMessage`): una racha es cierta sin importar cómo se reparta la cancha. Muchos grupos arman los equipos en la cancha; sin esta salida se quedaban sin aviso para siempre. Mismo mensaje que cuando alguien no tiene ningún cruce que llegue a `MIN_HEAD_TO_HEAD_MATCHES`.
- Ventana `[+30 min, +6h]`. El piso existe porque un aviso que llega cuando ya estás entrando a la cancha no sirve.
- **Solo TITULARES** (`isOnWaitlist !== true`) y con cuenta: al suplente que quizás ni juega, un "hoy tenés enfrente a Fulano" le llega de una charla que no es la suya. **Le llega a TODOS los que tengan un mensaje** (cruce o racha): antes había un tope del 60% sorteado, y se sacó a pedido — a los que les llega les gusta, y el que quedaba afuera del sorteo no entendía por qué.
- **El tono del cara a cara compara GANADOS contra PERDIDOS** (`describeVersus`, en `src/utils/versus.js` para `ProfileViewPage` y copiado en `functions/index.js` — ⚠️ mantener iguales). Antes salía de `ganados / jugados`: 2G-2E-2P se leía como derrota, y con 0 perdidos de 6 decía "perdés 0 de 6. Ponete las pilas" con carita triste. La frase muestra siempre el registro completo (`2 ganados, 2 empatados y 2 perdidos`).
- `hypeMsgSent` se marca **solo cuando se decide avisar**; mientras se espera a los equipos NO se marca, así el siguiente tick reintenta.
- ⚠️ **No acelerar el tick de esta tarea.** Ya se probó bajarlo a 5 y a 2 min para que el aviso llegue antes, y es plata tirada: el mensaje se manda entre 30 min y 6 horas antes del partido, así que nadie percibe la diferencia entre 7 y 12 minutos — pero el tick se paga las 24 horas, corra o no haya partidos (cada corrida es una lectura: 144/día a 10 min, 288 a 5 min, 720 a 2 min). Lo valioso del mecanismo es que el mensaje **diga la verdad sobre los equipos**, no que llegue rápido.

**Química / rivalidad (`users/{uid}/chemistry|rivalry/{otro}`)**: las mantiene `syncMatchPairs`, llamada desde `onPlayerStatsWritten`. **No es delta por trigger**: antes cada trigger sumaba before→after y escribía los dos lados del par, y como "Cargar resultado" guarda todas las `playerStats` en un batch, los triggers concurrentes contaban cada par **doble** (aparecían 4 empates con alguien con quien se empató 2), y editar el marcador sin cambiar equipos no movía nada. Ahora cada partido tiene un libro `chemLedger/state` con el aporte ya aplicado por par; en una **transacción** se recalcula el aporte que corresponde al estado actual y se aplica solo la diferencia — los triggers concurrentes se ponen en fila y los siguientes no escriben nada. ⚠️ Un partido con química cargada y **sin libro** volvería a sumarse entero: `recalcAllStats` reconstruye todo y siembra los libros, así que tiene que correrse una vez después de desplegar este cambio.
...
**Voto secreto + podio (MVP y Muralla)**: mientras la votación está ABIERTA nadie ve el recuento parcial — ni en pantalla ni desde la consola. `MatchMvpVoting.vue`/`MatchMurallaVoting.vue` muestran solo los nombres y el tilde del voto propio, los composables **no tienen `subscribeToVotes`**, y las reglas de `mvpVotes`/`murallaVotes` separan `get` (solo el voto propio) de `list` (denegado). El motivo no es privacidad sino **validez**: ver que uno va ganando 5 a 1 arrastra el voto del que todavía no votó, y ahí la votación deja de medir quién jugó mejor. Esconderlo solo en la UI hubiera sido maquillaje, por eso la regla también cambió.
- Al **cerrar**, la colección de votos queda legible (`matchData(matchId).get('mvpVotingClosed', false) == true`) y el componente arma el **podio de 3 puestos** con `fetchTally` (un `getDocs` de una sola vez, no un listener: un resultado cerrado no cambia más) + `buildPodium` (`src/utils/podium.js`). **Los empates comparten puesto**, igual criterio que el cierre — puede haber dos 🥇 y ningún 🥈, y no se desempata por orden alfabético porque sería inventar un resultado que la votación no dio.
- **Reabrir** (`reopenMvpVoting`/`reopenMurallaVoting`, botón "Reabrir votación" en los componentes): solo owner/admin del grupo o admin global (`assertCanReopenVoting` — el creador a secas no). Los votos **no se borran**; sí se borra el ganador (campo del partido + `mvp`/`muralla` en `playerStats`), porque mientras se vota no hay ganador. Estampa `votingReopenedAt`, que le da a la votación **24hs propias** (`REOPENED_VOTING_WINDOW_MS`) antes de que `runAutoCloseMatches` la vuelva a cerrar — también en partidos con `resultLocked`.
- El podio es **solo visual**: no toca `playerStats` ni los acumuladores. El ganador oficial sigue siendo el que fija la CF en `mvpUserId`/`murallaUserId`. Existe para que la misma votación reconozca a más de una persona — el que salió segundo por un voto hoy no se enteraba de nada. Funciona **retroactivamente** con todos los partidos ya cerrados, porque los votos nunca se borran.
...
matches/{matchId}
  ...
  teamsAssignedAt: Timestamp|null            ← sello de "equipos armados"; cada reasignación lo pisa
  hypeMsgSent: boolean                       ← el aviso previo ya salió (solo lo escribe la CF)
...
dropouts/{matchId}_{regId}     ← kind 'left'|'replaced', userId, matchId, hoursBeforeMatch, wasStarter
  ← SOLO lo escriben las CF; lo lee SOLO un admin global

matches/{id}/chemLedger/state  ← { pairs: { "uid|otro": "CW"|"RE"|... } } aporte YA sumado de ese partido a
                                 chemistry/rivalry. Solo CF (syncMatchPairs); nunca lo lee el cliente
...
| `resendMatchListNotification` | Callable | Botón manual "reenviar aviso de lista abierta" — ver abajo |
...
| `replaceRegistration` | Callable | Reemplaza una inscripción por otro miembro del grupo (ver arriba) |
| `reopenMvpVoting` / `reopenMurallaVoting` | Callable | Reabre una votación cerrada (owner/admin del grupo o admin global) |
| `onMatchDeletedCleanDropouts` | Firestore trigger | Borra las filas de `dropouts` de un partido borrado |
| `recalcAllStats` | Callable (admin-only) | Full rebuild of every user's `stats`/`statsByGroup` **and `chemistry`/`rivalry`** from all `playerStats` docs (collectionGroup scan), seeding each match's `chemLedger`. Needed because the delta accumulator can't recover results saved while `onPlayerStatsWritten` was undeployed. Triggered from the "Recalcular estadísticas" card in `AdminDashboardPage` |
...
| `runMatchHypeNotify` | 10 min | Aviso personalizado, una vez armados los equipos (ver arriba) |
| `runPostMatchReminder` | 10 min | A las 3hs de terminado: cargar stats y votar MVP/Muralla |
...
`tests/firestore.rules.test.js` — 118 tests de las reglas de seguridad contra el emulador (`npm run test:rules`, necesita Java). Cubren: aislamiento entre grupos (leer partidos y listas), el `limit` obligatorio al listar `matches`, quién puede borrar un partido, la rama de resultado que no debe poder cerrar listas, `memberCount`, el confinamiento del invitado anónimo a su partido, campos de perfil no escribibles (`stats`/`role`), `notificationPrefs`, calificar la descripción solo entre compañeros de grupo, los `reports`, los partidos públicos y sus postulaciones (quién publica, quién se postula, quién resuelve, el sondeo), las insignias (lectura pública, escritura negada a todos incluido el admin) la auto-promoción a admin/OG de un grupo, la votación de Muralla (voto propio, no al suplente, independiente de la de MVP, cerrada = no se vota más), el **voto secreto** de MVP y Muralla (abierta: cada uno lee solo su voto y nadie lista; cerrada: se puede listar, que es lo que alimenta el podio) y el historial cara a cara `chemistry`/`rivalry` (lectura abierta, escritura solo del backend) el registro de bajas `dropouts` (solo lo lee un admin global, nadie escribe) y el sello `teamsAssignedAt` (lo pone quien arma los equipos, no un miembro común, y por esa rama no se cuela ningún otro campo — `hypeMsgSent` incluido).
