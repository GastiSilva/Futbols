...
**Reenvío manual del aviso de lista abierta (`resendMatchListNotification`)**: botón "📣" en `MatchDetailPage`, junto a "Compartir lista", visible solo mientras `match.status === 'open'`. Existe porque los avisos automáticos (`match_open`, `match_reminder`) salen **una sola vez**; en un grupo donde la lista queda abierta toda la semana ese único push se pierde en el chat y nadie se acuerda de anotarse — esto la "revive" a mano. Manda a los miembros del grupo que todavía no están anotados (mismo patrón `sendFCMToGroupMembers` + `getRegisteredUserIds` que `match_reminder`/`low_signup_alert`).
...
- **Solo TITULARES** (`isOnWaitlist !== true`) y con cuenta, y el cupo (`HYPE_SHARE_OF_PLAYERS` = 0.6) se calcula sobre ellos: al suplente que quizás ni juega, un "hoy tenés enfrente a Fulano" le llega de una charla que no es la suya. Si sobran candidatos se **sortea** entre ellos, para que no le toque siempre a los mismos.
